...
def setupParserOptions():
    ...
    ap.add_argument('--oname',
                    default=None,
                    help="Provide the name of the output html. Default is\
             the basename of input file with prefix ls-.")
    ap.add_argument('-o',
                    '--odir',
                    default=None,
    ...
    ap.add_argument(
        ...
        help="Attributes to for the filtering slider.\
             Default is rlnAutopickFigureOfMerit.")

    args = vars(ap.parse_args())
    return args


def plot_overlay_picks(df, img, img_h, bin_num, level):
    ...
    out, bins = pd.cut(df[level], bin_num, retbins=True)
    ...
    for df in dfs:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                text=[
                    '{:0.3f}'.format(i) for i in df[level]
                ],
                hovertemplate='%{text}',
                name="",
                showlegend=False,
...
def main(**args):
    ...
    if args['skipdone'] and oname in os.listdir(odir):
        pass
    else:
        ...
        level = args['level']
        fig = plot_overlay_picks(df, img, img_h, bin_num, level)
