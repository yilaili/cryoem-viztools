...
Usage of star2df: star2df(starfile). Will return a dictionary (star_df, see above).
'''
...
def loop2df(loop):
    ...
    df.columns = keys
    df[keys] = df[keys].apply(pd.to_numeric, errors='ignore')
...
def star2df(starfile):
    with open(starfile) as f:
        star =[l for l in (line.strip() for line in f) if l and not l.startswith('#')] 
